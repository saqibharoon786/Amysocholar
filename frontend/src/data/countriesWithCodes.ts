// Countries with phone dial codes (E.164) - for Profile & Signup
...
export const countriesWithCodes: CountryCode[] = [
...
// Parse full phone (e.g. "+92 300 1234567") into dialCode and local number
export function parsePhone(fullPhone: string | undefined): { dialCode: string; number: string } {
  if (!fullPhone || !fullPhone.trim()) return { dialCode: "+92", number: "" };
  const trimmed = fullPhone.trim();
  const plusMatch = trimmed.match(/^(\+\d{1,4})\s*(.*)$/);
  if (plusMatch) return { dialCode: plusMatch[1], number: (plusMatch[2] || "").replace(/\s/g, "").trim() };
  if (/^\d/.test(trimmed)) return { dialCode: "+92", number: trimmed.replace(/\D/g, "").trim() };
  return { dialCode: "+92", number: trimmed };
}

// Build full phone for API
export function buildPhone(dialCode: string, number: string): string {
  const digits = number.replace(/\D/g, "").trim();
  if (!digits) return "";
  return `${dialCode.trim()} ${digits}`.trim();
}
